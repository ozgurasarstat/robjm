...
real<lower = 0> sigma0_Z;      // scale parameter of measurement error
vector[ncol_a] psi;
...
sigma_Z = to_vector(sigma0_Z * exp(a * psi)); 

//for(i in 1:ngroup){
//B[i, ] = Bstar[i, ] * sqrt(V[i]);
//}
for(i in 1:q){
B[, i] = to_vector(Bstar[, i]);
}
...
sigma0_Z ~ cauchy(0, priors_long[4]);

W ~ inv_gamma(delta/2, delta/2);
beta ~ cauchy(0, priors_long[5]);
psi ~ cauchy(0, priors_long[5]);
